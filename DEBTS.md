...
- **现状**:`WRITE_COMMANDS`/`NETWORK_EXTERNALS` 按命令名精确匹配
  (security.rs),重定向与 registry 内置命令有 `resolve_path(for_write)`
  路径级兜底(真边界),但外部进程(`git commit`、`npm install`、
...
### 插件供应链:capabilities 未强制、启动即执行(原审核 S-9)
...
- `test_ls_tilde_lists_home`:ls 排序比较器非全序,std smallsort panic
  ...
  (070 未触碰)。待修比较器(或换 sort_by_cached_key)。
- `examples_smoke` 的 cron-list/svc-status、`examples_parity` 的
  positional_arg:**auto-lang VM 在途回归**(engine.rs:1492 panic、
  for 循环字符串拼接损坏 —— 手工复现 `./x.tmp*.bakfind`),属姊妹仓
...
## 冻结临时文件留存(Plan 076,2026-08-28)

- **现状**:超限输出冻结时全文落 `%TEMP%/ash-freeze-<毫秒>.log`,不主动
  清理(依赖系统 temp 生命周期),无数量上限/LRU。
