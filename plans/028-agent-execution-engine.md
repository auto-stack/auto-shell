...
3. **Atom 的 21 种语义类型没暴露给 Agent** —— `ls` 返回 `FileList`、`ps` 返回 `ProcessList`,这些语义信息对 Agent 决策极有价值,但目前没暴露。
...
| `data.atom_type` | 对应 Atom 系统的 21 种类型 |
...
把 `ash-core/src/pipeline/atom.rs` 的 21 种 `AtomType` 暴露成 `kind` 标签,并定义每种的标准 JSON shape:
...
- Polars-like dataframe 抽象(复用 Atom 的 21 种语义类型,不另起炉灶)
- 流式处理(大于内存的数据集)
...
- `ash-core/src/pipeline/atom.rs` —— 21 种 Atom 语义类型
