...
| **MS3** | 脚本编程能力 | ASH 能写真实自动化/CI 脚本 | ✅ 完成（010/011）|
...
### 验收标准
- [x] `ash -c "ls | sort -w size" --json` 输出合法 JSON 数组
- [x] `ash -c "ls /nonexistent"` 退出码 1，stderr 有诊断，stdout 空
- [x] `ash -c "show data.csv | grep alice"` 非交互正确执行
- [x] 全量测试通过，无 panic（用模糊/边界输入验证）
- [x] stdout 只含数据（Agent 解析不被日志污染）
...
### 验收标准
- [x] `ash -c "rm -rf /" --sandbox /tmp` 被拦截
- [x] `--dry-run` 显示操作但不实际执行写
- [x] `--read-only` 下 `touch f` 被拒绝
- [x] `--audit` 生成完整命令日志
...
### 验收标准
- [x] 能写一个部署脚本（条件判断 + 循环 + 错误处理 + 函数）—— 各组件已就绪（fn/if/for/while/try-catch/system，Plan 010+011）
- [x] `ash deploy.ash` 端到端执行 —— 单元测试 + 手动脚本验证通过
- [x] try/catch 能捕获命令错误并恢复
...
1. **计划驱动**：每个 MS 拆成多个 Plan（如 `docs/plans/007-*.md`），先 plan 再 TDD 实施。
