...
### 第十五轮勘误:静默退出债确认仍活跃(2026-08-23 01:04 死亡实例)

第十四轮"未复现/缓解待观察"结论**被推翻**:复测所用的那只实例在
压力结束后约 01:04 静默退出(exit 1,无 panic、无 WER;后台任务通知
延迟 ~4h 才送达,一度误判为存活)。

现场证据:
- vm-out.log 末行 = iced 渲染器 flex-wrap 降级警告(Plan 412 §5 路径),
  即死亡发生在一次含 flex-wrap 的视图构建期间/之后;
- 死亡时间窗与最后一轮 MCP 密集探测(snapshot/state 连打,侧栏调试)
  重合 —— 与第五轮原始疑点(MCP 状态同步线程 × VM/渲染线程竞争)
  方向一致;
- 分钟级压力窗(大 Code 块 + 截图 + 状态查询)未能触发 —— 触发条件
  比原设想更窄(特定渲染路径 × 特定并发时序),或需要更长运行积累。

结论回滚为:**活跃债**。处治:①auto-lang 侧现由 Plan 419 agent 占用,
