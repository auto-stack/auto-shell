...
- 日期:2026-08-26
- 状态:**COMPLETE(Phase 1-3 全落地,含两轮冒烟迭代与 finish-plan 复审三遗漏修复 47a88ec);Phase 4 syntect 高亮延期(DEBTS 在册);人工验证项(resize/CJK 复制)留待日常使用覆盖**
- **finish-plan 复审记录(2026-08-26)**:
  ...
  - **复审发现(可action,已修 47a88ec)**:① 编辑器内 F1/F2/F3/F4、Alt+1..4 为死键
    → 改为 ExitThen(prefix) 退出并按内联语义转交(open_script_editor 内 F2=直接离开,
    Ctrl+O 内 F2=锁 AutoScript 重开;纯数字仍为文本);② 空缓冲 Ctrl+Enter → Run("")
    → 改为 Exit no-op;③ ASH_HARDWARE_CURSOR=1 定位缺行号 gutter 偏移
    (textarea 自证 screen_cursor().col 不含 gutter,widget.rs 同款 +digits+2 补偿)。
  ...
  - 绕道清单(随档):进入擦除仅清 1 行(折行输入残留,mod.rs 注释);viewport
    ...
    Ctrl+O 内按 F2 转开脚本编辑器时缓冲内容不保留(与内联 F 键丢弃尾随文本一致)。
- 实施记录(2026-08-26):
  ...
  - **冒烟迭代 #1(用户反馈,3d9e190)**:① 编辑器改为圆角边框输入框
    (Block::bordered(Rounded),模式名=顶标题、快捷键=底标题);② 进入时擦除
